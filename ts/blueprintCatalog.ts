...
export default class BlueprintCatalog {
  ...
  constructor(opts: BlueprintCatalogOptions = {
    ...
    this.options = opts;
    this.blueprintDictionary = new Dictionary({
      ignoreCase: opts.ignoreCase
    });
    this.hydratedBlueprints = new Dictionary({
      ignoreCase: opts.ignoreCase
    });
  }

  private blueprintDictionary: Dictionary = null;
  private hydratedBlueprints: Dictionary = null;
  private bpList = [];
   ...
   */
  loadSingleBlueprint(blueprint, blueprintName, progressCallback) {
   ...
   * @param {function} [progressCallback] Callback with the signature  function(blueprintName, loaded (boolean), message, blueprint)
   */
  loadBlueprints(block, progressCallback) {
   ...
   */
  extendBlueprint(orig, extendwith, inPlaceExtend?) {
   ...
   * @return {object}
   */
  getBlueprintFromInstance(instance) {
   ...
   */
  getBlueprintsDescendingFrom(baseBlueprintName, recurse) {
